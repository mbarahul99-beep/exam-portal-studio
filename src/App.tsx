...
export default function App() {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        ...
        console.warn("Could not load WhatsApp configuration:", err);
      }
    };
    loadSettings();
  }, []);
  ...
  const triggerPrint = (exam: Exam) => {
    ...
    setTimeout(() => {
      window.print();
      // Keep it mounted longer (e.g. 5000ms) on mobile or listen to afterprint
  ...
  const renderOmrCanvas = (canvas: HTMLCanvasElement, exam: Exam, isDemoFilled: boolean) => {
    ...
    const getQuestionLabel = (qNum: number): string => {
      ...
      const sec = exam.sections.find((s: any) => qNum >= s.qStart && qNum < s.qStart + s.qCount);
      if (!sec || !sec.subjectName || sec.subjectName.toUpperCase().includes('SUB') || sec.subjectName.toLowerCase() === 'subject' || sec.subjectName.toLowerCase() === 'general') {
        return pad;
      }
  ...
  const handleCopyExam = async (exam: Exam) => {
    try {
      const copiedExam: Exam = {
        title: `${exam.title} (Copy)`,
        className: exam.className,
        date: exam.date,
        status: 'private', // Duplicate as draft
        ...
        sectionsMarking: exam.sectionsMarking ? { ...exam.sectionsMarking } : undefined,
        rollNoDigits: exam.rollNoDigits,
        examSetsCount: exam.examSetsCount,
        showResultsToStudent: exam.showResultsToStudent,
        subjects: exam.subjects ? JSON.parse(JSON.stringify(exam.subjects)) : undefined,
        sections: exam.sections ? JSON.parse(JSON.stringify(exam.sections)) : undefined,
        answerKeys: exam.answerKeys ? JSON.parse(JSON.stringify(exam.answerKeys)) : undefined,
        difficulties: exam.difficulties ? { ...exam.difficulties } : undefined,
        loginOption: exam.loginOption,
        passcode: exam.passcode
      };

      const newExamId = await db.exams.add(copiedExam);
      
      // Sync the new copied exam to Hostinger MySQL Cloud
      await syncExamToCloud({ ...copiedExam, id: newExamId });

      // Copy MCQ questions associated with the original exam (for online exams)
      const originalQuestions = await db.questions.where('examId').equals(exam.id!).toArray();
      if (originalQuestions.length > 0) {
        const newQuestions = originalQuestions.map(({ id, ...qFields }) => ({
          ...qFields,
          examId: newExamId
        }));
        ...
        try {
          await fetch('/api/questions', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ examId: newExamId, questions: newQuestions })
          });
        } catch (err) {
      ...
      toast.innerText = `Exam "${exam.title}" successfully duplicated!`;
    ...
    <div className="app-container">
      ...
      <header className="mobile-header no-print" style={{ justifyContent: 'flex-start', gap: '16px', padding: '12px 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '2px' }}>
          <img src="/logo.png" alt="APEX Logo" style={{ height: `${logoHeight}px`, width: 'auto', objectFit: 'contain', marginRight: `-${logoHeight * 0.15}px` }} />
      ...
      <div className="no-print app-layout">
        ...
        <aside className={`sidebar ${mobileMenuOpen ? 'open' : ''}`}>
          <div className="sidebar-brand" style={{ display: 'flex', alignItems: 'center', gap: '2px', padding: '16px 20px', background: '#fff', borderBottom: '1px solid var(--border-color)' }}>
            <img src="/logo.png" alt="APEX Logo" style={{ height: `${logoHeight}px`, width: 'auto', objectFit: 'contain', marginRight: `-${logoHeight * 0.15}px` }} />
            <img src="/logo_name.png" alt="Institute APEX" style={{ height: `${logoNameHeight}px`, width: 'auto', objectFit: 'contain' }} />
          </div>
          ...
          <div style={{ width: '100%', maxWidth: '1000px', padding: '10mm 15mm', boxSizing: 'border-box', background: '#ffffff', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
            ...
            {activePrintJob.type === 'bulk-cards' && activePrintJob.students && (
              <div className="print-pages-wrapper">
                {chunkArray(activePrintJob.students, 9).map((pageStudents, pageIdx) => (
                  <div key={pageIdx} className="print-page" style={{
                    pageBreakAfter: pageIdx < Math.ceil(activePrintJob.students!.length / 9) - 1 ? 'always' : 'auto',
                    breakAfter: pageIdx < Math.ceil(activePrintJob.students!.length / 9) - 1 ? 'page' : 'auto',
                    display: 'grid',
                    ...
                    {pageStudents.map((student, idx) => (
                      <div key={idx} style={{
                        ...
                        <div style={{ textTransform: 'uppercase', fontSize: '0.65rem', fontWeight: '800', color: '#2563eb', letterSpacing: '1px', marginBottom: '8px', borderBottom: '2px solid #eff6ff', width: '100%', paddingBottom: '4px' }}>STUDENT IDENTITY CARD</div>
                        <img 
                          src={`https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=${encodeURIComponent(student.studentNum + ':' + student.className)}`} 
                          alt="QR"
                          style={{ width: '120px', height: '120px', minWidth: '120px', minHeight: '120px', marginBottom: '8px', objectFit: 'contain' }}
                        />
                        <div style={{ fontSize: '0.95rem', fontWeight: 700, color: '#0f172a', marginBottom: '2px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', width: '100%' }}>{student.name}</div>
                        {student.fatherName && (
                          <div style={{ fontSize: '0.78rem', color: '#64748b', fontWeight: 600, marginBottom: '4px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', width: '100%' }}>F-{student.fatherName}</div>
                        )}
                        <div style={{ fontSize: '0.75rem', color: '#64748b' }}>Class: <strong>{student.className}</strong></div>
                        <div style={{ fontSize: '0.75rem', color: '#64748b', marginTop: '2px' }}>Roll ID: <span style={{ fontWeight: 'bold', color: '#2563eb' }}>{student.studentNum}</span></div>
                      </div>
                    ))}
                  </div>
                ))}
            ...
            {activePrintJob.type === 'bulk-stickers' && activePrintJob.students && (
              <div className="print-pages-wrapper">
                {chunkArray(activePrintJob.students, 24).map((pageStudents, pageIdx) => (
                  <div key={pageIdx} className="print-page" style={{
                    pageBreakAfter: pageIdx < Math.ceil(activePrintJob.students!.length / 24) - 1 ? 'always' : 'auto',
                    breakAfter: pageIdx < Math.ceil(activePrintJob.students!.length / 24) - 1 ? 'page' : 'auto',
                    display: 'grid',
                    ...
                    {pageStudents.map((student, idx) => (
                      <div key={idx} style={{
                        ...
                        <img 
                          src={`https://api.qrserver.com/v1/create-qr-code/?size=90x90&data=${encodeURIComponent(student.studentNum + ':' + student.className)}`} 
                          alt="QR"
                          style={{ width: '85px', height: '85px', minWidth: '85px', minHeight: '85px', marginBottom: '4px', objectFit: 'contain' }}
                        />
